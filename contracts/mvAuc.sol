...
contract mvAuc {
    ...
    struct Bid{
        bytes32 blindedBid; // keccak256 encrypted bid to be revealed later ()
    ...
    uint public biddingEnd; //time limit to get bids in (still encrypted).
    ...
    mapping(address => Bid[]) public bids; //from address to array of sealed bids (structs)
    ...
    constructor(
        ...
        int[] _weights,  // signed weights for bid elements.
    ) public {
        beneficiary = _beneficiary;
        biddingEnd = now + _biddingTime;
        revealEnd = biddingEnd + _revealTime;
        weights = _weights;
        numWeights = _weights.length;
    }
    ...
    function bid(bytes32 _blindedBid) public payable onlyBefore(biddingEnd) {
        bids[msg.sender].push(Bid({ //recall bis is a mapping from addresses to a arrays of 'Bids' (structs)
    ...
    function reveal(
        uint[numWeights][] memory _values,
        bool[] memory _fake,
        ...
        onlyBefore(revealEnd) // before reveal time period is over
        {
            ...
            require(_values.length == length); //note: value is 2D array
            ...
            for (uint i=0; i < length; i++){
                ...
                require(_values[i].length == numWeights) //make sure the number of bid elements equal the number of weights
